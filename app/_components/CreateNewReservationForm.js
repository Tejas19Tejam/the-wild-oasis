...
function CreateNewReservationForm({ cabin, user }) {
  const { maxCapacity, numGuests, observations, regularPrice, discount, id } =
    cabin;
  ...
  async function handleCreateReservation(formData) {
    const formFieldObj = Object.fromEntries(formData.entries());
    ...
    await createBooking(createNewBooking);
